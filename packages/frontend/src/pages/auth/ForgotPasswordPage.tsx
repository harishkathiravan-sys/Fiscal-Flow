...
export default function ForgotPasswordPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setError('');
    setLoading(true);
    try { await authApi.forgotPassword(email); setSent(true); } catch (err: any) { setError(err.message || 'Something went wrong.'); } finally { setLoading(false); }
  };
      ...
      <div className="w-full max-w-sm">
        ...
        {sent ? (
          <div className="mt-6 space-y-4">
            <Alert variant="success" title="Email sent">If an account exists with <strong>{email}</strong>, you'll receive a reset link shortly.</Alert>
            <Button variant="secondary" className="w-full" onClick={() => { setSent(false); setEmail(''); }}>Send another email</Button>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="mt-8 space-y-4">
            <Input label="Email address" type="email" placeholder="you@company.com" value={email} onChange={(e) => setEmail(e.target.value)} required autoFocus />
